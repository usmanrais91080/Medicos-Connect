...
const MentalBoxBreathing = () => {
  const routeData = useRoute();
  const circleScale = useRef(new Animated.Value(1)).current;
  const [isAnimating, setIsAnimating] = useState(false);
  const [timer, setTimer] = useState(0);
  const [phase, setPhase] = useState('Take a moment and de-stress');
        ...
        setPhase(prevPhase => {
          switch (prevPhase) {
            ...
            case 'Hold your breath':
              return 'Breathe Out';
            case 'Breathe Out':
              return 'Hold your breath 2'; // intermediate phase to differentiate
            case 'Hold your breath 2':
              return 'Breathe In';
            default:
              return 'Breathe In';
          }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.breathe}>
        Box <Text style={styles.exercise}>Breathing</Text>
      </Text>
      <ScrollView>
        ...
        <Text style={styles.phase}>{phase.replace(' 2', '')}</Text>
...
export default MentalBoxBreathing;
