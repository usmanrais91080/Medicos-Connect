...
export const PaymentModal = (props) => {
    ...
    const handleWebViewNavigationStateChange = newNavState => {
        console.log("New function values>>>>>", newNavState)
                        ...
                        <View style={styles.modalContainer}>
                            <View style={{ paddingTop: "10%", marginHorizontal: "5%" }}>
                                <View style={{height:"80%"}}>
                                    {/* <Tier /> */}
                                    <Text style={{ marginHorizontal: "9%", ...styles.redText }}>{"Buy more Ampules"}</Text>
                                    <Text style={{ marginHorizontal: "19%", ...styles.grayText }}>{props.alertMessage}</Text>
                                    <FlatList data={props?.packages}
                                        contentContainerStyle={{ paddingTop: "10%" }}
                                        renderItem={({ item, index }) => {
                                            return (
                                                <View  style={{ borderRadius: 10, flexDirection: "row", justifyContent: "space-evenly", alignItems: "center", padding: 10, marginBottom: "5%" }} >
                                                    <Ampules height={30} width={30} />
                                                    <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                                                        <View style={{ marginRight: 2 }}>
                                                            <Text style={[styles.grayText1,{color: themeStyle.DASH_DARK,textAlign:'center'}]}>{item.ampules} <Text style={[styles.grayText1,{color: themeStyle.PRIMARY_TINT_COLOR,fontSize: 14,fontFamily:themeStyle.FONT_REGULAR}]}>Ampules</Text></Text>
                                                            <Text style={[styles.grayText2,{color: themeStyle.PRIMARY_TINT_COLOR,}]}>for ${item.price}</Text>
                                                        </View>
                                                        <TouchableOpacity onPress={() => props.payForAmpules(item._id)} style={{ height: 40, width: 70, marginLeft: "7.5%", backgroundColor: themeStyle.COLOR_RED, borderRadius: 10, justifyContent: "center", alignItems: "center" }}>
