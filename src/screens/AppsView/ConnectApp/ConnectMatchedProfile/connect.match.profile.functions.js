...
ConnectMatchProfileFunction.prototype.getMatchedProfiles = function (userDataToken) {
    console.log(userDataToken)
    return new Promise((resolve, reject) => {
        ConnectModuleServices.getMatchedProfiles(userDataToken)
...
ConnectMatchProfileFunction.prototype.acceptMatchedProfileRequest = function (userProfileId, userDataToken) {
    return new Promise((resolve, reject) => {
        ConnectModuleServices.acceptMatchedProfileRequest(userProfileId, userDataToken)
            .then((response) => { if (response.data.code == 200) resolve(response.data.data); else reject(response.data); })
            .catch((err) => { logError(err); reject(err); })
    })
}
 ...
 */
 ...
 ConnectMatchProfileFunction.prototype.rejectMatchedProfileRequest = function (userProfileId, userDataToken) {
    return new Promise((resolve, reject) => {
        ConnectModuleServices.rejectMatchedProfileRequest(userProfileId, userDataToken)
            .then((response) => { if (response.data.code == 200) resolve(response.data.data); else reject(response.data); })
            .catch((err) => { logError(err); reject(err); })
    })
}
